...
public class CategoryServiceImpl extends ServiceImpl<CategoryDao, CategoryEntity> implements CategoryService {
    ...
    public List<CategoryEntity> listWithTree() {
        ...
        List<CategoryEntity> level1Menus = entities.stream().filter(categoryEntity ->
            ...
            menu.setChildren(getChildres(menu, entities));
            return menu;
        }).sorted((menu1, menu2) -> {
...
//        //TODO 1.检查当前删除的菜单，是否在别处被引用，如果被引用，则不能删除
//     //   baseMapper.deleteBatchIds(asList);
//    }

    //递归查找所有的子菜单
    private List<CategoryEntity> getChildres(CategoryEntity root, List<CategoryEntity> all) {
        List<CategoryEntity> children = all.stream().filter(categoryEntity -> {
            return categoryEntity.getParentCid() == root.getCatId();
        }).map(categoryEntity -> {
            categoryEntity.setChildren(getChildres(categoryEntity, all));
